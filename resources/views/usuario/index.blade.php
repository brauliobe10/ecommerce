                ...
                <table class="min-w-full">
                    <thead>
                        <tr>
                            ...
                            <th scope="col" class="ui-th">Registro</th>
                    ...
                    <tbody>
                        @forelse ($usuarios as $user)
                            <tr class="transition-colors hover:bg-zinc-50 dark:hover:bg-zinc-800/50">
                                <td class="ui-td font-mono text-zinc-400 dark:text-zinc-500">{{ $user->id }}</td>
                                <td class="ui-td">
                                    <div class="flex items-center gap-3">
                                        <div class="flex size-9 items-center justify-center rounded-full bg-indigo-100 font-semibold text-indigo-700 dark:bg-indigo-500/15 dark:text-indigo-400">
                                            {{ $user->initials() }}
                                        </div>
                                        <span class="font-medium text-indigo-700 dark:text-indigo-400">{{ $user->name }}</span>
                                    </div>
                                </td>
                                <td class="ui-td">{{ $user->email }}</td>
                                <td class="ui-td">{{ $user->created_at?->format('d/m/Y') ?? '-' }}</td>
                                <td class="ui-td">
                                    <div class="flex items-center justify-center gap-2">
                                        <a href="{{ route('usuarios.edit', $user->id) }}" class="ui-btn-edit">
